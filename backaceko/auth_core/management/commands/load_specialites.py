...
from auth_core.models import SpecialiteMedicale  

class Command(BaseCommand):
    help = 'Charge les spécialités médicales depuis un fichier JSON'

    def handle(self, *args, **options):
        json_path = os.path.join(settings.BASE_DIR, 'auth_core', 'data', 'specialitemedical.json')
        
        try:
            with open(json_path, 'r', encoding='utf-8') as file:
                data = json.load(file)
                specialites = data.get('specialites_medicales', [])
                
                created_count = 0
                for spec in specialites:
                    obj, created = SpecialiteMedicale.objects.update_or_create(
                        nom=spec['nom'],
